...
namespace UnitTest
{
    inline bool check(bool value, bool expected) { return value == expected; }

    class TestAllocator;

    template <typename Expected, typename Actual> void checkEqual(TestResults& results, Expected const& expected, Actual const& actual, char const* const testName, char const* const filename, int const line, TestAllocator* allocator)
    {
        if (!(expected == actual))
        {
    ...
    template <typename Expected, typename Actual> void checkNotEqual(TestResults& results, Expected const& expected, Actual const& actual, char const* const testName, char const* const filename, int const line, TestAllocator* allocator)
    {
        if (expected == actual)
        {
            UnitTest::StringBuilder stringBuilder(allocator);
    ...
    template <typename Expected, typename Actual, typename Tolerance> bool areClose(Expected const& expected, Actual const& actual, Tolerance const tolerance) { return (actual >= expected - tolerance) && (actual <= expected + tolerance); }
    ...
    template <typename Expected, typename Actual> void checkArrayEqual(TestResults& results, Expected const& expected, Actual const& actual, int const count, char const* const testName, char const* const filename, int const line, TestAllocator* allocator)
    {
        bool equal = true;
    ...
    template <typename Expected, typename Actual, typename Tolerance> bool checkClose2(Expected const& expected, Actual const& actual, Tolerance const tolerance) { return ((actual >= expected - tolerance) && (actual <= expected + tolerance)); }
    ...
    template <typename Actual, typename Compare> void checkGreater(TestResults& results, Actual const& actual, Compare const& compare, char const* const testName, char const* const filename, int const line, TestAllocator* allocator)
    {
        if (!(actual > compare))
        {
            UnitTest::StringBuilder stringBuilder(allocator);
            stringBuilder << "Expected greater than ";
    ...
    template <typename Actual, typename Compare> void checkGreaterEqual(TestResults& results, Actual const& actual, Compare const& compare, char const* const testName, char const* const filename, int const line, TestAllocator* allocator)
    {
        if (!(actual >= compare))
        {
            UnitTest::StringBuilder stringBuilder(allocator);
            stringBuilder << "Expected greater than or equal to ";
            stringBuilder << compare;
            stringBuilder << " but was ";
            stringBuilder << actual;
            results.onTestFailure(filename, line, testName, stringBuilder.getText());
